...
Các ứng dụng Spring Boot thường gặp phải độ trễ cold start cao trong quá trình khởi tạo JVM và framework, nơi một lượng thời gian đáng kể được dành cho việc tải các class và thực hiện biên dịch Just-In-Time (JIT) mã Java bytecode. Bài đăng blog này sử dụng một ứng dụng Spring Boot làm ví dụ để truy xuất 10 bản ghi từ bảng UnicornEmployee trong cơ sở dữ liệu [Amazon RDS](https://aws.amazon.com/rds/) for PostgreSQL, trong đó mỗi bản ghi nhân viên bao gồm tên, địa điểm và ngày tuyển dụng.

Ứng dụng mẫu sử dụng [Amazon API Gateway](https://aws.amazon.com/api-gateway/) để kích hoạt một hàm AWS Lambda kết nối đến cơ sở dữ liệu thông qua RDS Proxy để trả về dữ liệu nhân viên. Mặc dù ứng dụng mẫu này sử dụng dữ liệu nhân viên giả để minh họa, các mẫu và kỹ thuật tối ưu hóa được thảo luận trong bài viết này có thể áp dụng cho các kịch bản thực tế với các mẫu truy cập dữ liệu tương tự.
...
Priming là quá trình tải trước các dependency và khởi tạo tài nguyên trong pha INIT, thay vì trong pha INVOKE, để tối ưu hóa hơn nữa hiệu năng khởi động với SnapStart. Điều này là cần thiết vì các framework Java sử dụng dependency injection sẽ tải các class vào bộ nhớ khi các class này được gọi một cách tường minh, điều này thường xảy ra trong pha INVOKE của Lambda. Bạn có thể chủ động tải các class bằng cách sử dụng [Java runtime hooks](https://docs.aws.amazon.com/lambda/latest/dg/snapstart-runtime-hooks-java.html), là một phần của dự án mã nguồn mở [CRaC](https://openjdk.org/projects/crac/) (Coordinated Restore at Checkpoint). Bài viết này minh họa cách sử dụng hook này, được gọi là `beforeCheckpoint()`, để "prime" các hàm Java đã bật SnapStart, theo hai cách:
...
Để tạo ra một danh sách các class cần thiết cho việc tải trước, bạn có thể sử dụng tùy chọn VM option sau, ghi danh sách ra một tệp có tên là `classes-loaded.txt`:
...
Thiết lập ứng dụng Spring Boot của bạn bằng cách sử dụng `aws-serverless-springboot3-archetype` như đã giải thích trong hướng dẫn [Quick Start Spring Boot3](https://github.com/aws/serverless-java-container/wiki/Quick-start---Spring-Boot3) của chúng tôi, thêm mã kết nối cơ sở dữ liệu, hoặc đơn giản là clone dự án mẫu từ [GitHub repository](https://github.com/aws-samples/lambda-priming-crac-java-cdk).
...
Thêm các dependency cho Spring Boot, AWS Lambda, và Kết nối Cơ sở dữ liệu vào tệp [pom.xml](https://github.com/aws-samples/lambda-priming-crac-java-cdk/blob/main/software/priming/pom.xml) của bạn. Dependency sau đây chứa các class cần thiết để sử dụng các CRaC runtime hook:
...
1. Triển khai interface `RequestHandler<UnicornRequest, UnicornResponse>` trong class handler của hàm Lambda.

2. Triển khai interface `Resource` của CRaC với hai method: `beforeCheckpoint()` và `afterRestore()`, định nghĩa các hành động được thực hiện trước khi Lambda tạo snapshot và sau khi snapshot được khôi phục.

3. Thêm invoke priming bằng cách tạo một đối tượng UnicornRequest với một request GET đến một endpoint cụ thể (ví dụ: `/unicorn`) và gọi method `handleRequest(unicornRequest, null)`.

Điều này đảm bảo rằng các luồng mã liên quan đến endpoint được chỉ định sẽ được biên dịch JIT và tối ưu hóa để thực thi nhanh hơn trong lần gọi đầu tiên sau khi snapshot được khôi phục.
...
1. Đảm bảo rằng các class ứng dụng của bạn có trong tệp `classes-loaded.txt` được tạo ra không làm thay đổi trạng thái trong quá trình khởi tạo tĩnh.

**Lưu ý:** tệp `classes-loaded.txt` được tạo ra chứa các mục class ở định dạng sau:
...
Chỉ trích xuất các tên class đủ điều kiện (fully qualified class names) từ mỗi dòng và loại bỏ thông tin ghi log bổ sung. Ví dụ:

```
software.amazon.awscdk.examples.unicorn.handler.ClassPriming
```

3. Sử dụng method tiện ích `ClassLoaderUtil.loadClassesFromFile()` để trích xuất các mục class đã được tạo.
...
4. Đọc một tệp (ví dụ: `/classes-loaded.txt`) chứa danh sách các class đã được tải trong quá trình thực thi của ứng dụng trong method `beforeCheckpoint()`.

5. Sử dụng method `Class.forName()` để tải và khởi tạo class, đảm bảo rằng nó đã sẵn sàng trong quá trình tạo snapshot.

**Lưu ý**: bằng cách tải trước các class này một cách có hệ thống, phương pháp Class priming giúp đơn giản hóa quá trình tối ưu hóa và giảm bớt sự phức tạp liên quan đến Invoke priming.
...
Việc triển khai sẽ xuất ra năm URL ở định dạng này:

```
# Endpoint ON_DEMAND (không có SnapStart)
...
Để tìm hiểu sâu hơn, hãy xem [GitHub repository](https://github.com/aws-samples/lambda-priming-crac-java-cdk) với mã nguồn ví dụ đầy đủ, bao gồm hướng dẫn thiết lập và các mẫu có thể tái sử dụng mà bạn có thể điều chỉnh cho các dự án của riêng mình. Để có thêm nhiều [ví dụ về ứng dụng Java chạy bằng Lambda](https://serverlessland.com/repos?services=lambda&runtime=Java), hãy truy cập [serverlessland.com](https://serverlessland.com/) và khám phá nhiều loại tài nguyên, hướng dẫn và các trường hợp sử dụng thực tế.